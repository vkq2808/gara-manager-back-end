...
export const handleRegister = async (req, res) => {
    ...
    let { email, password, firstName, lastName, phone, birth } = req.body;
    if (!email || !email) {
        console.log("Email and Password are required");
    ...
    const token = jwt.sign({ email, password, firstName: firstName.trim(), lastName: lastName.trim(), phone, birth, role: USER_ROLE }, process.env.REGISTER_SECRET_KEY, { expiresIn: '20m' });
